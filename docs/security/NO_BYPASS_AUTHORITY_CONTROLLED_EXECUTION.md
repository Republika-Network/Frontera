...
Fifty-five production-capable effect paths, enumerated from current source. Every one carries a stable `EP-` id and exactly one classification from §6.
...
| **EP-054** | **Execution resolution status query (P12)** — a trusted resolution authority asking its provider what an uncertain execution did | `AocEnterprise.executionReconciliation.reconcile()` (trusted in-process only) → `createExecutionReconciliationService` → the execution's **durably bound** `ExecutionResolutionAuthority.resolve()` — **host-written code**; Frontera ships no implementation and no network call for it | EXTERNAL — whatever read-only status query the host authority performs. **Never the original effect**: no `ExecutionAdapter`, exercise gate or execution service is reachable from P12, and the authority's contract forbids executing, resubmitting or mutating the original action | Explicit trusted call; a verified P11 attempt, the existing write-ahead claim, an eligible basis (no observation, or `unconfirmed`), the verified binding; at most one query per call, none when a resolution already stands; the answer is closed and normalized (SEC-INV-111 … SEC-INV-122) | n/a — the query carries the P11 attempt as context and its opaque `providerRef`; the answer has no amount, asset, grant or decision field | host-composed resolution authority → its provider | IN-PROCESS — **composition-gated**: exists only when a deployment enables `executionReconciliation` and composes an authority; no route, no SDK method | **DEPLOYMENT-GATED** |
| **EP-055** | **Exercise-control resolution row (P12)** — the second immutable P7 fact that stops a proven non-completion consuming | EP-054, **after** the P12 resolution is durable → `ExerciseControlReconciliationPort.reconcileResolution` (a fresh one-method object only the reconciliation service holds) → `BEGIN IMMEDIATE` in `sqlite-exercise-control-ledger.ts` | AUTHORITY MUTATION (A-28) — **permitting direction** for `confirmed-not-completed` (returns capacity in every bucket the reservation named); neutral for `confirmed-completed`. The original terminal event is never rewritten | Reachable only with a canonical P12 resolution digest; refused when the ledger's own history contradicts it; one row per reservation, append-only, verified before it can stop anything consuming; the same lock as admission (SEC-INV-117 … SEC-INV-121) | n/a — no provider | as EP-051 | IN-PROCESS — **composition-gated**: reconciliation and exercise controls both composed; never reached from HTTP | **DEPLOYMENT-GATED** |
| **EP-013** | Bounded-grant revocation | `AuthorityControlledExecutionService.revokeGrant()` (since P8, followed — after the store proved the revocation — by a write-only `grant.revoked` evidence report that is enqueued, never awaited, and whose failure is discarded) | AUTHORITY MUTATION (A-01) | Store idempotency; first revocation stands and is never re-dated | n/a | in-memory store | IN-PROCESS | **PROVEN — PATH LOCAL** |
...
| DEPLOYMENT-GATED | **21** | EP-002, EP-003, EP-006…EP-010, EP-020…EP-026, EP-028, EP-029, EP-046, EP-047, EP-048, EP-054, EP-055 |
| NON-EFFECTING | **5** | EP-001, EP-004, EP-005, EP-030, EP-045 |
| DEAD / UNREACHABLE | **2** | EP-027, EP-031 |
| **Total** | **55** | |

**Eight of fifty-five effect paths are under bounded-grant control.** That is the single most important number in this document, and every external statement about Frontera's execution control must be consistent with it.

*P13 did not change this number.* MPP challenge adaptation (`docs/architecture/ADR-MPP-CHALLENGE-AND-BUSINESS-IDEMPOTENCY.md`) is an **ingress transformation, not a new effect path**. `AocEnterprise.mppChallengePayments.prepareAndGovern()` turns a merchant's `Payment` challenge into an ordinary `GovernedActionIntent` and calls the same `GovernedActionOrchestrator.govern()` trusted in-process code could already call; every provider effect still occurs through EP-011 below the committed decision, the bounded grant, P10, P7, P11, P12 and the write-ahead claim, exactly as for EP-049. Its business-operation store writes are **PERSISTENCE** folded into that path (§4.1): they mutate no authority, gate nothing permissively and only *stop* a request before governance. P13 performs no network I/O, holds no adapter, grant store, ledger or P11/P12 writer, builds no credential and adds no route, SDK method or adapter invocation site (SEC-INV-123 … SEC-INV-137).

*P12 changed the denominator, not the numerator: eight of fifty-five.* The two additions are **deployment-gated trusted in-process paths**, not bounded-grant-controlled ones. EP-054 is the status query a host-composed resolution authority may make of its provider — inventoried as its own path because it is a provider-facing call that exists only because P12 does, and **explicitly not** the original effect path: P12 reaches no adapter, exercise gate or execution service, and Frontera ships no network call for it. EP-055 is the P7 resolution row — a local authority-state write in the permitting direction, split out from EP-053 because it is reached from reconciliation rather than from inside the bounded-grant gate, and reachable only after a durable, digest-bound P12 resolution. P12's own binding writes before the claim, and its resolution and Governance evidence writes, are **PERSISTENCE** folded into EP-049 and EP-054 (§4.1). No route, SDK method, adapter invocation site or retry was added (`docs/architecture/ADR-EXECUTION-RECONCILIATION-AND-RESOLUTION-AUTHORITY.md`).

*P11 did not change this number.* The durable execution outcome store (`docs/architecture/ADR-DURABLE-MONETARY-OUTCOMES.md`) adds no effect path. Its two writes, the attempt preparation before the write-ahead claim and the initial observation after the runtime returned, are **PERSISTENCE** writes folded into EP-049, exactly as its Governance Store references are (§4.1: persistence about an effect is not a second effect). They mutate no authority and gate nothing permissively. A preparation that cannot be written only *stops* an attempt before its claim, and the one read is the replay of an execution identity the claim already records, which never invokes an adapter (SEC-INV-102 … SEC-INV-110). No route, provider call, adapter invocation site, retry or reconciliation was added. The new SQLite file is a persistence site of EP-049, and like the P7 ledger and P8 stream it is not yet in portability v1 backup (P17).

*P8 did not change this number.* The canonical authority event stream (`docs/enterprise/AOC_CANONICAL_AUTHORITY_EVENT_STREAM.md`) adds no effect path: its appends are **PERSISTENCE (evidence)** writes folded into the paths that already produce the facts they report — EP-049 (the committed decision, grant issuance, claim, reservation facts and outcome) and EP-013 (revocation) — exactly as EP-049 already folds in its Governance Store references (§4.1: persistence about an effect is not a second effect). They mutate no authority and gate no path: no check on any path reads the stream (SEC-INV-080), a failed append changes no outcome (SEC-INV-081), and no authority path waits for an append at all — facts are enqueued and projected afterwards, so a stuck stream cannot sit between the durable execution claim and the provider crossing (SEC-INV-088). No route, provider call, adapter invocation site or authority write was added.
...
| EP-055 `reconcileResolution` (P12) | Only from the reconciliation service, only after a durable P12 resolution, bound to its digest; refused on contradictory history; append-only, one row per reservation |
...
| Exercise-control ledger (P7) | EP-051, EP-052, EP-053, EP-055 | Direct in-process ledger reference; direct file write + re-seal | **PARTIALLY** |
...
### EP-054, EP-055 — Execution resolution status query and P7 resolution row (P12)
- **CLAIM:** with `executionReconciliation` composed, a resolution is obtained only from the execution's durably bound authority, only through the trusted in-process reconciliation service, at most once per explicit call and never when one already stands; it never executes, resubmits or retries the original effect and never dereferences a `providerRef`; capacity returns (EP-055) only after the resolution is durable, bound to its digest, and never when the ledger's own history contradicts it; ordinary customer replay reaches neither path.
- **EVIDENCE:** `execution-reconciliation-boundaries.test.ts` (no execution reach, no Kernel reach, narrow P7 capability, orchestrator holds no authority), `execution-reconciliation-e2e.test.ts` (flagship, claim-only, restart, crash gaps, corruption, adversarial answers, concurrency), `execution-resolution-store.test.ts`, `exercise-reservation-resolution-contract.ts` (both ledgers), `exercise-control-resolution-sqlite.test.ts` (worker-thread races).
- **CLASSIFICATION:** **DEPLOYMENT-GATED.**
- **SCOPE:** executions of the governed-action path that P11 left uncertain. Nothing here extends to any other EP.
- **EXCEPTIONS:** the resolution authority is trusted host code — its compromise can falsely return or retain capacity (SEC-TRUST-009); no cross-store atomicity between the resolution and EP-055 (fails toward availability loss).
- **BYPASS CONDITIONS:** in-process code holding a host-supplied ledger or resolution store; a filesystem writer who rewrites and re-seals the resolution store or the ledger (unkeyed digests, SEC-TRUST-002).
- **DEPLOYMENT ASSUMPTIONS:** D-A3, D-A4, D-A8; a local filesystem that honours SQLite locking.
