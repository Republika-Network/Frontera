...
- **Status as of CORE-01:** the durable store now also holds a signed revocation-state commitment, so a database-only writer can no longer remove a committed revocation (SEC-INV-124; this closes the delete-row-and-clear-pointer gap MASTER-00 found in the two-record cross-check). The "host-supplied durable store that loses revocations" shape is refused where durability is configured: a `persistence.provider = 'sqlite'` Host accepts only the authenticated durable store (SEC-INV-125). Snapshot rollback across restart remains open (GS-002 → CORE-07). The path-local proof and EP-011's classification are unchanged.
