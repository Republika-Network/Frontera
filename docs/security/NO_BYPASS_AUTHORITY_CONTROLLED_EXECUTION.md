...
| **EP-013** | Bounded-grant revocation | `AuthorityControlledExecutionService.revokeGrant()` (since P8, followed — after the store proved the revocation — by a write-only `grant.revoked` evidence append whose failure is discarded) | AUTHORITY MUTATION (A-01) | Store idempotency; first revocation stands and is never re-dated | n/a | in-memory store | IN-PROCESS | **PROVEN — PATH LOCAL** |
