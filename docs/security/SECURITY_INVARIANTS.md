...
| SEC-INV-047 (**EMERGENCY-05**) | An adapter-scoped stop is enforced after trusted routing and before the child adapter is invoked, and is never reported as a provider rejection. | COMPONENT-LOCAL to `createExecutionAdapterRegistry` | RUNTIME + TEST-ENFORCED | `execution-adapter-registry.ts`; `execution-adapter-registry.test.ts`; `emergency-control-governed-action.test.ts` | Only the typed `EmergencyControlWithheldError` maps to `withheldBy: 'emergency-control'`; every other adapter throw stays `ADAPTER_ERROR`. |
...
| SEC-INV-054 (**EMERGENCY-11**) | An active emergency control cannot be removed silently. The durable store keeps an append-only, sequenced, chained event history, a current-state projection pointing at the event that produced it, and a head row anchoring the latest sequence, count and digest; a read cross-checks all three, so a deleted control row, a deleted event, a deleted head, a re-pointed or stale projection each report `unavailable`. A release is an explicit recorded transition; a row that vanishes is damage, never consent. | COMPONENT-LOCAL to `createSqliteEmergencyControlStore` | RUNTIME + TEST-ENFORCED | `emergency-control-record.ts`; `sqlite-emergency-control-store.ts` (`verifiedHead`, `verifiedControl`); the deletion regressions in `emergency-control-durability.test.ts` | **Unkeyed digests.** Detects partial deletion and mutation; does not stop a writer who re-seals every record consistently, and cannot detect wholesale replacement of the database file (GS-002, SEC-TRUST-002). |
