...
- Companion documents: `docs/security/TRUST_BOUNDARIES.md` (canonical: trust zones, boundary register, privileged-asset register), `docs/security/THREAT_MODEL_V1.md`, `docs/security/SECURITY_HARDENING_V1.md`, `docs/kernel/AOC_KERNEL_INVARIANTS_V1.md` (Kernel-scoped), `docs/architecture/ADR-AUTHORITY-CONTROL-LAYERING.md`.
...
| SEC-INV-021 | No dynamic code evaluation: no `eval(`, `new Function(`, dynamic `import(`, or `vm`. | LAYER-LOCAL to layers C, D, E, the execution runtime, **and the Kernel** | STRUCTURAL + TEST-ENFORCED | `context-layer-boundaries.test.ts:118`; `obligation-layer-boundaries.test.ts:160`; `grant-layer-boundaries.test.ts:228`; `execution-layer-boundaries.test.ts:270-279`; Kernel coverage added in `security-invariants.test.ts` | Not proven for `packages/` or `apps/`. `apps/agent-passport-web` contains one constant-specifier `require('stripe')`. |
...
- Whether to converge this path onto bounded-grant exercise, or to record it as a permanent documented exception, is Prompt 3's decision. This document does not pre-empt it.
...
| SEC-TRUST-003 | **Authoritative state durability.** The default bounded-grant store is in-memory, and recognition, authority-graph, approval and handshake state are in-process `Map`s. | Authorization state is lost on restart. Grant loss fails closed (`GRANT_EXERCISE_NOT_FOUND`); capability-token revocation is **not durable**. | `in-memory-bounded-grant-store.ts` (the only `BoundedGrantStorePort` implementation); `capability-token-service.ts:47`; `kernel-provider-composition.ts:60-87`; `AOC_AUTHORITY_CONTROLLED_EXECUTION.md` §15 |
...
|---|---|---|---|
| SEC-INV-U01 | No authority artifact is trusted without a verified signature from a key the application process cannot read. | ASPIRATIONAL-UNIMPLEMENTED. Unkeyed SHA-256 digests only; no signatures anywhere in `src/`. Attachment point already identified: a detached signature over `serializeBoundedGrant`'s output, recorded beside the grant. | Prompt 5 |
...
3. **System-wide adapter gating.** SEC-INV-011 is PATH-LOCAL. It does not hold for `enforce()` or Sovereign Access (SEC-INV-010, SEC-INV-019).
...
4. **New effect paths must declare themselves.** Any new path capable of producing an external effect must be added to §5 with its own guarantees and non-guarantees before it ships. A path absent from §5 is not covered by any invariant here.
