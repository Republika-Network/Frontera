...
| **A-28** Exercise-control reservations (P7, bounded-grant path) | SQLite at `exerciseLedger.sqlitePath` — always durable when composed — or a host-supplied ledger | D-06 (file: its own, never shared with A-01, A-03 or A-27) | `ExerciseControlGate` only: `reserve` / `settle` / `release`, reachable solely inside the exercise gate (EP-051 … EP-053) | the same gate, inside its admission transaction | Deleting or re-sealing reservations returns spent aggregate capacity — more effects than the host's limits allow. Row, rule, terminal-event and bucket-head digests make an un-resealed edit or a partial deletion fail closed; they cannot stop a writer who re-seals consistently (SEC-TRUST-002). Reserving can only withhold |
