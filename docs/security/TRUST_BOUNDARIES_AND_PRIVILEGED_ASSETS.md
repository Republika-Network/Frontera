...
| D-06a | Bounded-grant database file + its backups | PRIVILEGED — **outside application control** | `boundedGrant.sqlitePath`, default `.data/bounded-grants.sqlite` | **New in Prompt 4.** Anyone with filesystem write access can replace records or roll the store back; no application check prevents it (D-GS1, R-GS-03). Restoring a snapshot taken before a revocation restores revoked authority, with every integrity check passing (GS-002). Since CORE-01, deleting revocation rows no longer does — only a restore of a *captured* earlier signed state does, and a running process detects that |
...
| Host-composed resolution authorities (P12) | Answering what an uncertain execution's provider did (SEC-TRUST-009) | Authorization, amounts, assets, grants, budgets, choosing which execution they answer for |
...
| **A-31** Execution resolutions (P12, governed-action lifecycle, when `executionReconciliation` is composed) | SQLite at `executionResolution.sqlitePath` under `persistence.provider = 'sqlite'`, process-local otherwise, or a host-supplied store; plus one resolution row per reservation inside the P7 ledger file (A-28) | D-06 (its own file, never A-30's) | The orchestrator's pre-claim binder (bindings only); the trusted in-process reconciliation service (adoption bindings, definitive resolutions, and the P7 row through a one-method capability) | the store / ledger, inside one `BEGIN IMMEDIATE` transaction per write | **Execution fact with one permitting-direction consequence.** A verified `confirmed-not-completed` resolution returns P7 capacity (SEC-INV-117); nothing else reads it to decide. Forging a binding could route reconciliation to another composed authority; forging a resolution could mislead replay or return capacity — digests over the closed contract make an un-resealed edit fail verification, and a record that fails is never replayed or used for capacity (SEC-INV-119); a consistent re-seal is not detected (P20). Deleting it leaves executions unresolved and capacity consumed, never retried |
...
| **A-32** `AOC_ENTERPRISE_AUTHORITY_SIGNING_KEY_PEM` — the authority signing **private** key (Ed25519, PKCS#8) | env ⇒ **application process memory** | D-17 ⇒ D-06 | operator | the signer only, inside `issue` / `revoke` | **Mint bounded grants and revocations that verify perfectly.** The strongest authority-forging asset introduced by Prompt 5, and the reason AA-001 is HIGH. Unlike A-13/A-14 this is genuinely asymmetric, so a verifier holding only A-33 cannot mint — but the process holding this one can. Removing it from process memory is deferred external key custody |
| **A-33** `AOC_ENTERPRISE_AUTHORITY_VERIFICATION_KEYS` — the trusted verification registry (public keys + key ids) | env ⇒ frozen in-process registry | D-17 ⇒ D-06 | operator | durable store read path | **Not a secret — but the root of trust.** A writer who can replace it installs their own key and makes their own artifacts authentic (AA-002). Removing an entry makes every artifact signed by that key unreadable — a key-trust decision, **not** a revocation (§13.1 of `AUTHORITY_ARTIFACT_AUTHENTICITY.md`) |

*A-32 and A-33 were A-27 and A-28 in the historical authority-authenticity change (security Prompt 5); they were renumbered when it was forward-ported (PRE-00), because current main already assigns A-27 to emergency controls and A-28 to exercise-control reservations.*
...
**A-33 is the opposite case, and the contrast is the point.** It is also public material, and it genuinely does verify: the bounded-grant signatures are asymmetric, so the public half is sufficient to check authority and insufficient to mint it. That is why integrity of A-33 matters for *availability and trust* (replace it and you control what is believed) while confidentiality of it does not matter at all. A-13/A-14, being HMAC secrets, have the reverse profile and no verifier/signer separation to speak of.
...
|---|---|---|---|---|
| `AuthorityArtifactSigner` | A-32 | `issue`, `revoke`, new-store genesis and commitment re-attestation at open (CORE-01) | **yes** | no |
| `AuthorityArtifactVerifier` | A-33 | the authoritative read path | no | **yes** |
...
**The limit of that split, stated plainly.** It is a boundary between *types and call sites*, not between *processes*. Both halves are constructed by the same composition root and live in the same process, so a process-level attacker holds A-32 regardless of which interface was handed where. What the split buys today is that a database-only writer cannot forge, and that no code on the read path can be made to sign without a type error. What it does **not** buy is protection from anything that can read this process's memory. Deferred external key custody (legacy label: Prompt 6) is what would turn the type boundary into a process boundary.
...
|---|---|---|---|---|---|---|
| `GrantIssuanceService.issueGrant` | A-01 | In-process host only; commit guard re-proves eligibility inside the store transaction (SEC-INV-016) | **No route** (SEC-INV-027) | **Yes when the durable store is configured**, otherwise no | Grant carries `sourceDigest`, correlation; the persisted record carries its own envelope digest **and, on the durable path, a detached signature naming the key that produced it** | Mint arbitrary bounded grants. Requires A-32 to produce ones the durable read path will accept |
| `AuthorityControlledExecutionService.revokeGrant` | A-01 | In-process host only | **No route** | **Yes when the durable store is configured**, at durability equal to the grant's by construction (SEC-INV-035) — **and now dependent on signer availability: a revocation that cannot be signed is refused, never written unsigned (AA-004)** | Integrity-protected **and signed** revocation record, cross-referenced from the grant row | Suppress revocation — requires producing a valid signature over a revocation-state commitment that omits it (SEC-INV-124, CORE-01), or restoring a captured earlier signed state (GS-002). Deleting both records, as MASTER-00 showed sufficed before CORE-01, now fails closed. A-32 still defeats it |
...
| Filesystem write to store files | **Yes** | Process user / operator | Software → persistence | Digests + chains detect; `verify` must be run. The durable bounded-grant store additionally requires a trusted signature (SEC-INV-123) | Prompt 5 (historical) — bounded-grant store only; other stores open |
| Host compromise | **Yes** | Attacker with host access | Everything | None in software | Prompt 17 |
| DB compromise | **Yes** | Anyone with file access | Persistence | Detective only; re-sealing defeats it — except in the durable bounded-grant store, where a database-only writer cannot produce a trusted signature (SEC-INV-123) | Prompt 5 (historical) — bounded-grant store only; other stores open |
...
> **SEC-TRUST-009 — A resolution authority is trusted to answer provider truth (P12).** When `executionReconciliation` is composed, each host-written `ExecutionResolutionAuthority` is trusted to say whether an execution it is durably bound to completed. Frontera constrains what crosses the port and which authority may answer which execution; it cannot verify the answer. A compromised authority can falsely return or falsely retain P7 capacity; it cannot authorize, change an amount, issue a grant or reach the original effect. Stated in `SECURITY_INVARIANTS.md` §6 and `ADR-EXECUTION-RECONCILIATION-AND-RESOLUTION-AUTHORITY.md` §3.
