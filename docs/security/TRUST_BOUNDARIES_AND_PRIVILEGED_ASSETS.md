...
| **A-32** MPP business operations (P13, governed-action lifecycle, when `mppChallengePayments` is composed) | SQLite at `mppBusinessOperation.sqlitePath` under `persistence.provider = 'sqlite'`, process-local otherwise, or a host-supplied store | D-06 (its own file) | The trusted in-process MPP challenge-payment service (one `record` capability); the read-only P14 challenge-context reader | the store, inside one `BEGIN IMMEDIATE` transaction per write | **Business identity, not authority.** Maps a caller-named purchase to one deterministic governed request and keeps its immutable challenge history; nothing that decides, issues, reserves or routes reads it. Forging an operation could change which terms a future P14 credential is built from — but the governed request's terms are the Kernel's, and a different digest is a conflict; an un-resealed edit fails verification and is refused, never read as absent (SEC-INV-136); a consistent re-seal is not detected (P20). Deleting it cannot mint a second request: the request id is derived, not stored. Holds no credential or secret (SEC-INV-133) |
...
> **SEC-TRUST-010 — MPP method normalizers and merchant mapping are trusted to interpret (P13).** When `mppChallengePayments` is composed, each host-written `MppChallengeMethodNormalizer` is trusted to interpret its method's `request` — base units and currency into exact P9 money — and `resolveCounterparty` to map a merchant reference to the right Frontera counterparty; `selectChallenge` is trusted host policy. Frontera fences what crosses them (closed answers, data descriptors, P9, no authority field) but cannot verify the interpretation. A wrong one proposes wrong terms, which are then governed — the Kernel, P10 ceilings and P7 limits still bound them — but not detected. None can authorize, name the business operation or reach an adapter. Stated in `SECURITY_INVARIANTS.md` §6 and `ADR-MPP-CHALLENGE-AND-BUSINESS-IDEMPOTENCY.md` §5.
